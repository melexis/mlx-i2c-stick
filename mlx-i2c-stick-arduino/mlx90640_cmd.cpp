...
void
mlx90640_show_calibration_parameters(uint8_t sa)
{
  ...
  paramsMLX90640 *p = &mlx->mlx90640_;
  Serial.printf("kVdd = %d\n", p->kVdd);
  Serial.printf("vdd25 = %d\n", p->vdd25);
  Serial.printf("KvPTAT = %f\n", p->KvPTAT);
  Serial.printf("KtPTAT = %f\n", p->KtPTAT);
  Serial.printf("vPTAT25 = %d\n", p->vPTAT25);
  Serial.printf("alphaPTAT = %f\n", p->alphaPTAT);
  Serial.printf("gainEE = %d\n", p->gainEE);
  Serial.printf("tgc = %f\n", p->tgc);
  Serial.printf("cpKv = %f\n", p->cpKv);
  Serial.printf("cpKta = %f\n", p->cpKta);
  Serial.printf("resolutionEE = %d\n", p->resolutionEE);
  Serial.printf("calibrationModeEE = %d\n", p->calibrationModeEE);
  Serial.printf("KsTa = %f\n", p->KsTa);
  Serial.printf("ksTo[0] = %f\n", p->ksTo[0]);
  Serial.printf("ct[0] = %d\n", p->ct[0]);
  Serial.printf("alpha[0] = %d\n", p->alpha[0]);
  Serial.printf("alphaScale = %d\n", p->alphaScale);
  Serial.printf("offset[0] = %d\n", p->offset[0]);
  Serial.printf("kta[0] = %d\n", p->kta[0]);
  Serial.printf("ktaScale = %d\n", p->ktaScale);
  Serial.printf("kv[0] = %d\n", p->kv[0]);
  Serial.printf("kvScale = %d\n", p->kvScale);
  Serial.printf("cpAlpha[0] = %f\n", p->cpAlpha[0]);
  Serial.printf("cpOffset[0] = %d\n", p->cpOffset[0]);
  Serial.printf("ilChessC[0] = %f\n", p->ilChessC[0]);
  Serial.printf("brokenPixels[0] = %d\n", p->brokenPixels[0]);
  Serial.printf("outlierPixels[0] = %d\n", p->outlierPixels[0]);
}
...
cmd_90640_init(uint8_t sa)
{
  ...
  // mlx90640_show_calibration_parameters(sa);
...
void
cmd_90640_ee(uint8_t sa, uint16_t *ee_data, uint16_t *ee_count, uint16_t *ee_start_address, char const **error_message)
{ // read all the EEPROM!
  ...
  *ee_count = 832;
  if (MLX90640_DumpEE(sa, ee_data) < 0)
  {
    *error_message = MLX90640_ERROR_COMMUNICATION;
    return;
  }
